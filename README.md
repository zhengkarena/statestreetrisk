...
OpenAI API key, click **Test connection**. The key is stored in `localStorage` on this
machine only and is sent to `api.openai.com` directly from the browser. See
...
The **Settings** panel handles the OpenAI API key with a test-connection ping and a
...
| **Direct browser → OpenAI** | A simple `Authorization: Bearer` header gets us to a fully functional AI demo with no proxy server. Production deployment would proxy through the API layer (see architecture below). |
...
flowchart LR
    ...
    subgraph ai["AI Layer"]
        ANP[OpenAI API proxy<br/>key in Vault · rate limits · audit]
        AN[OpenAI GPT]
    end
...
- **AI Co-Pilot** — OpenAI key moves to Vault. Calls go through a managed proxy that
...
└── src/
    ...
    │   ├── ai.js                     # OpenAI fetch wrapper + typed AIError
...
- **API key in `localStorage`.** The OpenAI key the user pastes into Settings is stored
  in browser `localStorage` and sent directly to `api.openai.com` from the page. OpenAI
  officially recommends against direct browser calls; this is acceptable on a personal demo
  machine but the production version would proxy AI calls through the API layer (see
  architecture above) so the key stays in a secrets manager and gets rate limiting, audit,
  and PII redaction.
